...
console.log("Longitude:", lon, "Type:", typeof lon);
    ...
    if (encodedCity) {
      url = `https://api.openweathermap.org/data/2.5/weather?q=${encodedCity}&units=metric&appid=ff23cbc5ec14267bbc0a2fe52332547a`;
    } else if (lat && lon) {
      url = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&appid=ff23cbc5ec14267bbc0a2fe52332547a`;
    } else {
    ...
    const forecast = await axios.get(
      `https://api.openweathermap.org/data/2.5/forecast?lat=${coord.lat}&lon=${coord.lon}&units=metric&appid=ff23cbc5ec14267bbc0a2fe52332547a`
    );
    setForeCastData(forecast.data);

    const UV = await axios.get(
      `https://api.openweathermap.org/data/2.5/uvi?lat=${coord.lat}&lon=${coord.lon}&appid=ff23cbc5ec14267bbc0a2fe52332547a`
    );
